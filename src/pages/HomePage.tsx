...
import { ClockIcon, StarIcon, MapPinIcon, SearchIcon, ChevronDownIcon, PhoneIcon } from 'lucide-react';
...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(location => location.name.toLowerCase().includes(searchTerm.toLowerCase()) || location.address.toLowerCase().includes(searchTerm.toLowerCase()));
    }
  ...
  const states = ['All States', ...new Set(locations.map(location => location.state))];
      ...
      <section className="py-12 md:py-20 px-4 bg-gradient-to-b from-yellow-50/80 to-white">
        <div className="container mx-auto">
          ...
          >
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="relative">
                ...
                <ChevronDownIcon size={18} className="absolute right-3 top-3.5 text-gray-400 pointer-events-none" />
              </div>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {(showAllLocations ? filteredLocations : filteredLocations.slice(0, 6)).map((location, index) => (
              <motion.div
                key={location.id}
                ...
                <div className="p-4 md:p-6">
                  ...
                  <div className="space-y-2 md:space-y-3">
                    <div className="flex items-start">
                      <MapPinIcon size={16} className="text-red-500 mt-1 mr-2 flex-shrink-0" />
                      <p className="text-gray-600 text-xs md:text-sm">{location.address}</p>
                    </div>
                    
                    <div className="flex items-center">
                      <PhoneIcon size={16} className="text-red-500 mr-2 flex-shrink-0" />
                      <a href={`tel:${location.phone}`} className="text-gray-600 hover:text-red-600 transition-colors text-xs md:text-sm">
                        {location.phone}
                      </a>
                    </div>
                    
                    <div className="flex items-center">
                      <ClockIcon size={16} className="text-red-500 mr-2 flex-shrink-0" />
                      <p className="text-gray-600 text-xs md:text-sm">{location.hours}</p>
                    </div>
                  </div>
                  
                  <div className="mt-4 md:mt-5 flex justify-between items-center">
                    <div className="flex items-center">
                      <div className={`w-2 h-2 rounded-full ${location.isOpen ? 'bg-green-500' : 'bg-red-500'} mr-2`} />
                      <span className={`text-xs md:text-sm font-medium ${location.isOpen ? 'text-green-600' : 'text-red-600'}`}>
          ...
          {filteredLocations.length > 6 && (
            ...
            >
              ...
              >
                {showAllLocations ? 'Show Less Locations' : 'Show All Locations'}
                <ChevronDownIcon size={16} className={`ml-2 transition-transform duration-300 ${showAllLocations ? 'rotate-180' : ''}`} />
              </button>
            </motion.div>
          )}
        </div>
                ...
                <div className="space-y-4 md:space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-3 md:pb-4 border-b border-gray-100 gap-2">
                    <div>
                      ...
                      <p className="text-gray-600 text-sm md:text-base">Monday - Friday</p>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-3 md:pb-4 border-b border-gray-100 gap-2">
                    <div>
                      ...
                      <p className="text-gray-600 text-sm md:text-base">Saturday - Sunday</p>
                    </div>
                    <div className="bg-yellow-100 px-3 md:px-4 py-2 rounded-lg">
                      <p className="text-red-700 font-medium text-sm md:text-base">
                        11:00 AM - 11:00 PM
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                      ...
                      <p className="text-gray-600 text-sm md:text-base">Special Hours</p>
                    </div>
                    <div className="bg-yellow-100 px-3 md:px-4 py-2 rounded-lg">
                      <p className="text-red-700 font-medium text-sm md:text-base">
                        11:00 AM - 10:00 PM
            ...
            {[
              ...
              >
                <div className="flex items-center mb-4">
                  ...
                  <span className="ml-2 text-gray-500 text-sm md:text-base">5.0</span>
                </div>
                ...
                <div className="flex items-center">
                  <div className="w-10 h-10 md:w-12 md:h-12 bg-red-100 rounded-full flex items-center justify-center mr-3 md:mr-4">
                    <span className="text-red-600 font-bold text-sm md:text-base">{testimonial.initials}</span>
                  </div>
                  <div>
