...
export class SecurityManager {
    ...
    private validateText(text: string): boolean {
        ...
        return text.length <= 1000 && !xssPatterns.some(pattern => pattern.test(text));
    }
    ...
    isSafeURL(url: string): boolean {
        try {
            ...
            return this.config.allowedOrigins.some(origin =>
                urlObj.origin === origin || urlObj.hostname === origin.replace(/^https?:\/\//, '')
            );
        } catch {
            return false;
        }
...
export const useInputValidation = () => {
    const validateEmail = (email: string) => securityManager.validateInput(email, 'email');
    const validatePhone = (phone: string) => securityManager.validateInput(phone, 'phone');
    const validateText = (text: string) => securityManager.validateInput(text, 'text');
    const validateNumber = (number: string) => securityManager.validateInput(number, 'number');
